...
// ─────────────────────────────────────────────────────────────────────────
// Standalone-install plumbing: when packaged, redirect all relative file ops
// to a writable user directory and point Playwright at the bundled chromium.
// In dev mode (`npm run desktop`), behave just like before.
// ─────────────────────────────────────────────────────────────────────────
function prepareUserEnvironment() {
  if (app.isPackaged) {
    // ~/Documents/CNMOOC Downloader/ — visible to users, has write permission.
    userDataPath = path.join(app.getPath('documents'), 'CNMOOC Downloader');
    fs.mkdirSync(userDataPath, { recursive: true });
    process.chdir(userDataPath);
    // Bundled chromium lives next to the app, inside the installer's resources.
    process.env.PLAYWRIGHT_BROWSERS_PATH = path.join(process.resourcesPath, 'pw-cache');
  } else {
...
global.cnmoocElectronLogin = function electronLogin() {
  return new Promise((resolve, reject) => {
    ...
    function cleanup(ok, err) {
      ...
      if (ok) resolve(storageStatePath);
      else reject(err);
    }
